...
class QueryChemblFunction(BiobuddyFunction):
    def __init__(
        ...
        super().__init__(
            "query_chembl",
            "Query Chembl database to find the molecules of interest. If asked for specific molecule, will return the single molecule like nicotine or aspirin.",
            parameters,
        )
    ...
    def execute(self, arguments: Dict[str, Any]) -> FunctionCall:
        ...
        request = None
        if order:
            request = external_data_query_microservice.ChEMBLMoleculeRequest(
                filters=filters, order_by=order, limit=max_results
            )
        else:
            request = external_data_query_microservice.ChEMBLMoleculeRequest(
                filters=filters, limit=max_results
            )

        print(request)
        molecules = self._chembl_microservice.query_query_chembl_post(
            ch_embl_molecule_request=request
        ).molecules
        ...
        for molecule in molecules:
            ...
            molecule.pref_name = (
                molecule.pref_name if molecule.pref_name else molecule.chembl_id
            )
            data.append(
                ChemBLData(
                    content=sdf_content,
        ...
        return FunctionCall(
            ...
            arguments=[
                FunctionParam(name="filters", value=filters),
                FunctionParam(name="order_by", value=order),
                FunctionParam(name="max_results", value=max_results),
            ],
