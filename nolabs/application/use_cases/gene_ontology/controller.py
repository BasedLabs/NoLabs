...
from nolabs.application.use_cases.gene_ontology.di import \
...
@router.post("/jobs/run/{job_id}", summary="Start job")
async def start_job(
    feature: Annotated[RunJobFeature, Depends(GeneOntologyDependencies.run_job)],
    job_id: UUID,
) -> JobResponse:
...
async def get_job(
    job_id: UUID,
    feature: Annotated[GetJobFeature, Depends(GeneOntologyDependencies.get_job)],
) -> JobResponse:
    return await feature.handle(job_id=job_id)
...
async def setup_job(
    ...
    feature: Annotated[SetupJobFeature, Depends(GeneOntologyDependencies.setup_job)],
) -> JobResponse:
...
async def get_job_status(
    job_id: UUID,
    feature: Annotated[
        GetJobStatusFeature, Depends(GeneOntologyDependencies.get_job_status)
    ],
