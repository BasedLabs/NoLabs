...
from nolabs.application.use_cases.msa_generation.di import \
    MsaGenerationDependencies
...
async def get_job(
    job_id: UUID,
    feature: Annotated[GetJobFeature, Depends(MsaGenerationDependencies.get_job)],
) -> JobResponse:
    return await feature.handle(job_id=job_id)
...
async def get_job_status(
    job_id: UUID,
    feature: Annotated[
        GetJobStatusFeature, Depends(MsaGenerationDependencies.get_job_status)
    ],
...
async def setup_job(
    ...
    feature: Annotated[SetupJobFeature, Depends(MsaGenerationDependencies.setup_job)],
) -> JobResponse:
