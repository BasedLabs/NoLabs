...
from nolabs.application.use_cases.localisation.di import \
    LocalisationDependencies
from nolabs.application.use_cases.localisation.use_cases import (
    GetJobFeature, GetJobStatusFeature, SetupJobFeature)
...
@router.post("/jobs/run/{job_id}", summary="Start localisation job")
async def run_job(
    feature: Annotated[RunJobFeature, Depends(LocalisationDependencies.run_job)],
    job_id: UUID,
) -> JobResponse:
...
async def get_job(
    job_id: UUID,
    feature: Annotated[GetJobFeature, Depends(LocalisationDependencies.get_job)],
) -> JobResponse:
    return await feature.handle(job_id=job_id)
...
async def setup_job(
    ...
    feature: Annotated[SetupJobFeature, Depends(LocalisationDependencies.setup_job)],
) -> JobResponse:
...
async def get_job_status(
    job_id: UUID,
    feature: Annotated[
        GetJobStatusFeature, Depends(LocalisationDependencies.get_job_status)
    ],
