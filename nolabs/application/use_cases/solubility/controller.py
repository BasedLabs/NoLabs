...
async def run_job(
    feature: Annotated[RunJobFeature, fastapi.Depends(SolubilityDependencies.run_job)],
    job_id: UUID,
) -> JobResponse:
...
async def get_job(
    job_id: UUID,
    feature: Annotated[GetJobFeature, fastapi.Depends(SolubilityDependencies.get_job)],
) -> JobResponse:
    return await feature.handle(job_id=job_id)
...
async def setup_job(
    ...
    feature: Annotated[
        SetupJobFeature, fastapi.Depends(SolubilityDependencies.setup_job)
    ],
) -> JobResponse:
...
async def get_job_status(
    job_id: UUID,
    feature: Annotated[
        GetJobStatusFeature, fastapi.Depends(SolubilityDependencies.get_job_status)
    ],
