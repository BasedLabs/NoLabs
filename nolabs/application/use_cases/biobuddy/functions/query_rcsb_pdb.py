...
class QueryRCSBPDBFunction(BiobuddyFunction):
    def __init__(self, rcsb_pdb_query_microservice: external_data_query_microservice.DefaultApi):
        parameters = [
            FunctionParameterDefinition(name="query",
                                        type="string",
                                        required=True,
        ...
        super().__init__("query_rcsb_pdb", "Query RCSB PDB database to find entries based on specific conditions",
                         parameters)
    ...
    def execute(self, arguments: Dict[str, Any]) -> FunctionCall:
        ...
        response = self._rcsb_pdb_query_microservice.fetch_fetch_fastas_by_complex_query_post(body=query)
        ...
        return FunctionCall(function_name="query_rcsb_pdb", arguments=[FunctionParam(
            name="query_json",
            value=query)], data=FunctionCallReturnData(files=data))
