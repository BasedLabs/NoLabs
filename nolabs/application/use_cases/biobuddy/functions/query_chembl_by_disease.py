...
class QueryChemblByConditionFunction(BiobuddyFunction):
    def __init__(
        ...
        super().__init__(
            "query_chembl_by_condition",
            "Query Chembl database by disease/condition to find the "
    ...
    def execute(self, arguments: Dict[str, Any]) -> FunctionCall:
        ...
        request = None
        if order:
            request = external_data_query_microservice.DrugIndicationRequest(
        ...
        molecules = self._chembl_microservice.query_query_chembl_by_condition_post(
            request
        ).drugs
        ...
        return FunctionCall(
            function_name="query_chembl_by_condition",
            arguments=[
