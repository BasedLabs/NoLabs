...
from nolabs.application.use_cases.protein_design.di import \
...
async def get_job(
    job_id: UUID,
    feature: Annotated[GetJobFeature, Depends(ProteinDesignDependencies.get_job)],
) -> JobResponse:
    return await feature.handle(job_id=job_id)
...
async def setup_job(
    ...
    feature: Annotated[SetupJobFeature, Depends(ProteinDesignDependencies.setup_job)],
) -> JobResponse:
...
async def get_job_status(
    job_id: UUID,
    feature: Annotated[
        GetJobStatusFeature, Depends(ProteinDesignDependencies.get_job_status)
    ],
