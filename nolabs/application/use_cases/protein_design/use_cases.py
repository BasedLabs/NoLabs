...
from nolabs.application.use_cases.protein_design.api_models import (
...
class SetupJobFeature:
    ...
    async def handle(self, request: SetupJobRequest) -> JobResponse:
        ...
        job_name = JobName(
            request.job_name if request.job_name else "New protein design job"
        )
        ...
        job: ProteinDesignJob = ProteinDesignJob.objects(
            Q(id=job_id.value) | Q(name=job_name.value)
        ).first()

        if not job:
            job = ProteinDesignJob(id=job_id, name=job_name, experiment=experiment)
        ...
        try:
            ...
            if response.errors and not response.pdbs_content:
                raise NoLabsException(
                    ErrorCodes.protein_design_run_error, response.errors
                )
