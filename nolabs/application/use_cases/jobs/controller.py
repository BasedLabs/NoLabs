...
@router.get("/metadata", summary="Get all jobs metadata by experiment")
async def jobs_metadata(
    experiment_id: UUID,
    feature: Annotated[GetJobsMetadataFeature, Depends(JobDependencies.jobs_metadata)],
) -> List[GetJobMetadataResponse]:
    return await feature.handle(experiment_id=experiment_id)


@router.get("/{job_id}/metadata", summary="Get job metadata by experiment")
async def job_metadata(
    job_id: UUID,
    feature: Annotated[GetJobMetadataFeature, Depends(JobDependencies.job_metadata)],
) -> GetJobMetadataResponse:
    return await feature.handle(job_id=job_id)


@router.delete("/{jod_id}", summary="Delete job")
