...
class GetJobStatusFeature:
    ...
    async def handle(self, job_id: UUID) -> GetJobStatusResponse:
        ...
        if folding_backend == FoldingBackendEnum.esmfold:
            response = self._esmfold.is_job_running_job_job_id_is_running_get(
        ...
        if folding_backend == FoldingBackendEnum.esmfold_light:
            response = self._esmfold_light.is_job_running_job_job_id_is_running_get(
                job_id=str(job.iid.value)
            )
            return GetJobStatusResponse(
                running=response.is_running, result_valid=job.result_valid()
        ...
        if folding_backend == FoldingBackendEnum.rosettafold:
            response = self._rosettafold.is_job_running_job_job_id_is_running_get(
                job_id=str(job.iid.value)
            )
            return GetJobStatusResponse(
                running=response.is_running, result_valid=job.result_valid()
...
class RunJobFeature:
    ...
    def __init__(
        ...
        if not esmfold and not esmfold_light and not rosettafold:
            raise NoLabsException(
                ErrorCodes.folding_run_error,
                "You must specify at least one folding backend",
            )
    ...
    async def handle(self, job_id: UUID) -> JobResponse:
        ...
        try:
            ...
            for protein in job.proteins:
                ...
                pdb_content, errors = self.request_factory(
                    job_id=job.iid,
                    sequence=sequence,
            ...
            job.set_result(result=[(protein, pdb) for protein, pdb in result])
    ...
    def request_factory(
        self, job_id: JobId, sequence: str, folding_backend: FoldingBackendEnum
    ) -> Tuple[str, List[str]]:
        if folding_backend == FoldingBackendEnum.esmfold:
        ...
        if folding_backend == FoldingBackendEnum.rosettafold:
            response = self._rosettafold.run_folding_run_folding_post(
                job_id=job_id.value, fasta=sequence, _request_timeout=(1000.0, 1000.0)
            )
