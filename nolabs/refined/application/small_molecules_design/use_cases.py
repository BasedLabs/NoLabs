...
class SetupJobFeature:
    ...
    async def handle(self, request: SetupJobRequest) -> JobResponse:
        try:
            ...
            protein = Protein.objects.with_id(id=request.protein_id)

            if not protein:
                raise NoLabsException(ErrorCodes.protein_not_found)

            job = SmallMoleculesDesignJob.objects.with_id(id=request.job_id)
            created = False
            if not job:
            ...
            if created or job.center_x != request.center_x or \
                    job.center_y != request.center_y or \
                    job.center_z != request.center_z or \
                    job.size_x != request.size_x or \
                    job.size_y != request.size_y or \
                    job.size_z != request.size_z or \
                    job.batch_size != request.batch_size or \
                    job.minscore != request.minscore or \
...
class RunSamplingStageJobFeature:
    ...
    async def handle(self, job_id: UUID):
        try:
            job: SmallMoleculesDesignJob = SmallMoleculesDesignJob.objects.with_id(id=job_id)
