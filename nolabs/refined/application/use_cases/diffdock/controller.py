...
from nolabs.refined.application.use_cases.diffdock.api_models import (JobResponse, GetJobStatusResponse,
                                                                     SetupJobRequest)
from nolabs.refined.application.use_cases.diffdock.di import DiffDockDependencies
from nolabs.refined.application.use_cases.diffdock.use_cases import RunJobFeature, GetJobFeature, GetJobStatusFeature, \
    SetupJobFeature

router = APIRouter(
    prefix='/api/v1/diffdock',
    tags=['Diffdock']
)


@router.post('/jobs/run/{job_id}',
             summary='Start diffdock job')
async def start_job(
        feature: Annotated[
