...
from prefect import task, get_client, flow
from prefect.exceptions import ObjectNotFound

from nolabs.infrastructure.mongo_connector import mongo_connect
from nolabs.infrastructure.settings import settings
from nolabs.infrastructure.log import logger
from workflow.data import JobRunData, ComponentData
...
async def cleanup_orhpan_task_run_ids():
    ...
    for job in JobRunData.objects(task_run_id__ne=None).only('id', 'task_run_id'):
        ...
        if not exists:
            logger.info('Task run not found for job, cleaning up',
...
async def cleanup_orhpan_flow_run_ids():
    ...
    for cd in ComponentData.objects(flow_run_id__ne=None).only('id', 'flow_run_id'):
        ...
        if not exists:
            logger.info('Flow run not found for component, cleaning up',
