...
def run_docking(request: RunDiffDockPredictionRequest) -> RunDiffDockPredictionResponse:
    ...
    with tempfile.NamedTemporaryFile(
    ...
    ) as protein_file, tempfile.NamedTemporaryFile(
        delete=False, suffix=".sdf", mode="w+"
    ) as ligand_file, tempfile.NamedTemporaryFile(
        ...
        subprocess.run(
            [
                "python",
                "/app/DiffDock/esm/scripts/extract.py",
                "esm2_t33_650M_UR50D",
                "data/prepared_for_esm.fasta",
                "data/esm2_output",
                "--repr_layers",
        ...
        result = subprocess.run(
            [
                ...
                csv_file.name,
                "--out_dir",
                "results/user_predictions_small_new",
                "--inference_steps",
                str(request.inference_steps),
                "--samples_per_complex",
                str(request.samples_per_complex),
                "--batch_size",
        ...
        for result_dir in glob.glob(os.path.join(results_dir, "index*")):
            if os.path.isdir(result_dir):
                for sdf_file in glob.glob(os.path.join(result_dir, "*.sdf")):
                    ...
                    scored_affinity_match = re.search(
                        "Affinity:\s*([\-\.\d+]+)", scored_stdout.decode("utf-8")
                    )
                    scored_affinity = (
                        float(scored_affinity_match.group(1))
                        if scored_affinity_match
                        else None
                    )
                    ...
                    minimized_affinity_match = re.search(
                        "Affinity:\s*([\-\.\d+]+)", minimized_stdout.decode("utf-8")
                    )
                    minimized_affinity = (
                        float(minimized_affinity_match.group(1))
                        if minimized_affinity_match
                        else None
                    )
        ...
        response = RunDiffDockPredictionResponse(
            pdb_contents=pdb_contents, sdf_results=sdf_results
        )
