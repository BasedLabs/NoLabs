...
class FastaReader:
    ...
    def get_ids2seqs(self, fasta_contents: str | List[str]) -> List[AminoAcid]:
        ...
        if isinstance(fasta_contents, str):
            fasta_contents = [
                l for l in [l.strip() for l in fasta_contents.split("\n")] if l
            ]
        ...
        return [
            AminoAcid(name=sequence_id.split("|")[0], sequence=sequence)
...
class FastaWriter:
    ...
    def write_single_fasta(
        self, sequence_id: str, sequence: str, file_path: str
    ) -> None:
