  ...
  <q-card>
    ...
    <q-card-section>
      <div class="row items-center">
        ...
        />
        <div class="text-h6">
          AdaptyvBio Protein Binding Screening Job
        </div>
      </div>
    </q-card-section>
...
import {
  getProteinBindingScreeningEstimates,
  getProteinBindingScreeningJob,
  listProteinBindingScreeningTargets,
  sendProteinBindingScreeningRequest,
  setupProteinBindingScreeningJob,
} from '../../refinedApi';
...
export default defineComponent({
  ...
  methods: {
    async fetchJobParameters() {
      try {
        const job: JobResponse = await getProteinBindingScreeningJob(
          this.jobId
        );
    ...
    async fetchPriceEstimate() {
      ...
      try {
        const response = await getProteinBindingScreeningEstimates(this.jobId);
    ...
    async updateJob() {
      try {
        await setupProteinBindingScreeningJob(
          this.jobId,
          this.numDesigns,
          this.numAA,
    ...
    fetchTargets(val, update) {
      listProteinBindingScreeningTargets(val)
    ...
    async submitJob() {
      try {
        await sendProteinBindingScreeningRequest(this.jobId);
