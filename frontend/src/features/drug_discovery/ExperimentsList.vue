  ...
  <q-page padding>
    ...
    <div v-else class="experiment-list">
      <div
        ...
        class="experiment-tab row q-mb-md"
      >
        <div @click="openExperiment(experiment.experiment_id)">
          <div
            class="col experiment-info"
            @click="openExperiment(experiment.experiment_id)"
          >
            <div class="experiment-name">{{ experiment.experiment_name }}</div>
            <div class="experiment-date">
              {{ formatDate(experiment.experiment_date) }}
            </div>
          </div>
          <div class="col-auto">
            <q-btn
              flat
              icon="more_vert"
              @click.stop="showMenu(experiment.experiment_id)"
            >
              <q-menu>
                <q-list>
                  <q-item
                    clickable
                    @click="changeExperimentName(experiment.experiment_id)"
                  >
                    <q-item-section>Change Name</q-item-section>
                  </q-item>
                  <q-item
                    clickable
                    @click="deleteExperiment(experiment.experiment_id)"
                  >
