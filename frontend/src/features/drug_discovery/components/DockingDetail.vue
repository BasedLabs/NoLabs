    ...
    <q-card v-if="isDocking">
        <q-card-section>
            <q-stepper v-model="dockingStep" vertical color="info" animated>
                <q-step color="info" :name="1" title="Predict MSA" :done="dockingStep > 1">
                    <q-linear-progress :value="progress[0]" striped color="blue" />
                </q-step>
                <q-step color="info" :name="2" title="Get Folded Structure" :done="dockingStep > 2">
                    <q-linear-progress :value="progress[1]" striped color="green" />
                </q-step>
                <q-step color="info" :name="3" title="Get Binding Pocket" :done="dockingStep > 3">
                    <q-linear-progress :value="progress[2]" striped color="orange" />
                </q-step>
                <q-step :name="4" color="info" title="Run Docking" :done="dockingStep > 4">
                    ...
                    <q-card v-if="showResults">
                        ...
                        <q-card-actions align="right">
                            <q-btn flat label="Download Results" color="primary" @click="downloadResults" />
        ...
        viewResults() {
            ...
            this.$nextTick(() => {
                if (!this.resultsViewer) {
                    this.resultsViewer = new NGL.Stage(this.$refs.resultsViewContainer, { backgroundColor: "black" });

                    this.resultsViewer.loadFile('/7NB4_pred_ligand.sdf').then(component => {
                        component.addRepresentation('ball+stick');
                        component.autoView();
                    });

                    this.resultsViewer.loadFile('/7NB4_pred_protein.pdb').then(component => {
