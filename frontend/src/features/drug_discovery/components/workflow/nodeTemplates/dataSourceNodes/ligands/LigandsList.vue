<template>
    <q-list bordered style="width: 100%;" class="bg-black rounded-borders">
        ...
        <q-item v-for="ligand in filteredLigands" :key="ligand.id" clickable v-ripple class="q-mb-sm"
            @click="showLigandDetailDialog(ligand.id)">
    ...
    <q-dialog v-model="ligandDetailDialogVisible">
        <q-card>
            <q-card-section>
                <LigandDetail :ligand-id="selectedLigandId" v-if="selectedLigandId" />
            </q-card-section>
            <q-card-actions align="right">
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            selectedLigandId: null as string | null,
            fileModel: [],
    ...
    methods: {
        showLigandDetailDialog(ligandId: string) {
        ...
        async deleteLigand(ligandToDelete: LigandMetadataResponse) {
            this.$q.loading.show({
        ...
        async downloadLigand(ligand: LigandMetadataResponse) {
            try {
                const ligandContent = await getLigandContent(ligand.id) as LigandContentResponse;

                if (!ligandContent.sdf_content) {
                    console.error('SDF file content is missing for this ligand.');
                    return;
                }

                const sdfContent = ligandContent.sdf_content;
                const blob = new Blob([sdfContent], { type: 'chemical/x-mdl-sdfile' });
                const url = window.URL.createObjectURL(blob);

                const a = document.createElement('a');
                a.href = url;
                a.download = `${ligandContent.name}.sdf`;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                a.remove();
            } catch (error) {
                console.error('Error fetching ligand content for download:', error);
            }
        },
    },
    unmounted() {
