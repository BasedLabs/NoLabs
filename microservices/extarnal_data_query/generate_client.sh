...
DOCKER_IMAGE_NAME="rcsb_pdb_query"

sudo docker build -t $DOCKER_IMAGE_NAME -f build/Dockerfile .
# Run the Docker container in the background
# Replace the Docker image name with the appropriate one
sudo docker run -d --name rcsb_pdb_query -p 5739:5739 rcsb_pdb_query --host=0.0.0.0 --port=5739
...
npx @openapitools/openapi-generator-cli generate \
    ...
    -g python \
    -o ./client \
    --additional-properties=packageName=rcsb_pdb_query_microservice
