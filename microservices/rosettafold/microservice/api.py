...
from microservice.api_models import (IsJobRunningResponse,
                                     RunRosettaFoldResponse)
from microservice.services import RosettaService

app = FastAPI(title="RoseTTAFold API", version="0.1")

from microservice.loggers import logger


@app.post("/run-folding")
async def run_folding(
    job_id: str | None = None,
    fasta: Annotated[bytes, File()] = None,
    a3m: Annotated[bytes, File()] = None,
) -> RunRosettaFoldResponse:
    """
    ...
    if fasta is None and a3m is None:
        return RunRosettaFoldResponse(
            errors=["You must specify either fastsa or a3m"], pdb_content=None
        )
