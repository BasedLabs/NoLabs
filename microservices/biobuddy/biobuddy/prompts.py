...
def generate_strategy_prompt(tools_description: str, query: str, available_components: str, current_workflow: str) -> str:
    return (f"Given the available tools ({tools_description}), decide whether a direct reply is sufficient "
            ...
            f"If a user asks to pull some data (specific proteins/ligands) in the query, reply with a plan of all actions required based on the query and tools descriptions in the format (have as many actions as needed): "
            ...
            f"DON'T GENERATE THE PLAN IF IT'S NOT NEEDED, i.e. if a user asks to just adjust a workflow or a simple general question. DON'T GENERATE PLAN IF IT IS NOT NEEDED (i.e. if a user does not ask to pull specific proteins/ligands"
