...
def run_pdb_fixer(parameters: RunPdbFixerRequest) -> RunPdbFixerResponse:
    ...
    try:
        ...
        return RunPdbFixerResponse(
            pdb_content=None, errors=["Unable to run pdb fixer. Internal error", str(e)]
        )
    finally:
...
def run_gromacs_simulation(
    parameters: RunGromacsSimulationsRequest,
) -> RunSimulationsResponse:
    output_pdb = f"{uuid.uuid4()}.pdb"
...
def run_pdb_simulation(parameters: RunPdbSimulationsRequest) -> RunSimulationsResponse:
    ...
    try:
        ...
        forcefield = ForceField(
            *[parameters.force_field.value, parameters.water_force_field.value]
        )
...
def generate_gromacs_files(parameters: GenGroTopRequest) -> GenGroTopResponse:
    ...
    try:
        ...
        program = [
            ...
            output_top,
            "-ff",
            parameters.force_field.value,
            "-water",
            parameters.water_force_field.value,
        ]
        ...
        return GenGroTopResponse(
            gro=None, top=None, errors=[res.stderr.decode("utf-8")]
        )
    except Exception as e:
        ...
        return GenGroTopResponse(
            gro=None,
            top=None,
            errors=["Unable to generate .gro and .top files from this pdb", str(e)],
        )
    finally:
        _remove_file_if_exists(input_pdb)
