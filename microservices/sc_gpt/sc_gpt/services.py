...
def classify_cells(
    request: ReferenceMappingRequest, checkpoint_path="checkpoints/scGPT_human"
) -> ReferenceMappingResponse:
    assert os.path.exists(
        request.query_path
    ), f"Query path {request.query_path} does not exist"
    assert os.path.exists(
        request.reference_path
    ), f"Reference path {request.reference_path} does not exist"
    assert os.path.exists(
        checkpoint_path
    ...
    adata_concat.obs["is_ref"] = ["Query"] * len(test_embed_adata) + [
        "Reference"
    ] * len(ref_embed_adata)
    adata_concat.obs["is_ref"] = adata_concat.obs["is_ref"].astype("category")

    adata_concat.obs[request.cell_type_key] = adata_concat.obs[
        request.cell_type_key
    ].astype("category")
    adata_concat.obs[request.cell_type_key] = adata_concat.obs[
        request.cell_type_key
    ].cat.add_categories(["To be predicted"])
    adata_concat.obs[request.cell_type_key][
        : len(test_embed_adata)
    ] = placeholder_cell_type
...
def get_embeddings(
    request: EmbedRequest, checkpoint_path="checkpoints/scGPT_human"
) -> EmbedResponse:
    assert os.path.exists(
        request.dataset_path
    ), f"Dataset path {request.dataset_path} does not exist"
    assert os.path.exists(
        checkpoint_path
    ), f"Checkpoint path {checkpoint_path} does not exist"
