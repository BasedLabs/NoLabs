...
from reinvent.api_models import (ConfigurationResponse, LogsResponse,
                                 ParamsRequest, ParamsResponse,
                                 SamplingSizeRequest, SmilesResponse)
from reinvent.services import Reinvent, RunType
from starlette.responses import FileResponse

app = FastAPI(title="Reinvent4 API", version="0.0.1")
...
async def sampling(config_id: str, request: SamplingSizeRequest):
    """
    Generate new ligands based on the provided config id.
    """
    reinvent_instance = Reinvent()
    return await reinvent_instance.run(
        config_id,
        RunType.SAMPLING_SCORING,
        sampling_size=request.number_of_molecules_to_design,
    )
...
async def save_params(
    ...
    """
    reinvent_instance = Reinvent()
    await reinvent_instance.save_params(
        pdb_file=pdb_file,
