...
class Log:
    ...
    def run_solubility_prediction_request(
        self, request: RunSolubilityPredictionRequest
    ):
        d = request.as_log_dict()
        _logger.info("Run solubility request", extra=d)

    def run_solubility_prediction_response(
        self, response: RunSolubilityPredictionResponse
    ):
