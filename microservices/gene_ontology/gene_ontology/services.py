...
from gene_ontology.api_models import (GoConfidenceResponse,
...
class GeneOntologyPrediction:
    ...
    def load_model(self):
        ...
        self.model = SimpleGOMultiLayerPerceptron(input_dim=640, num_classes=200).to(
            self.device
        )
        self._load_model_state_dict()
        self.model.eval()
        self.labels = np.load(
            os.path.join(models_dir, "go_labels_200.npy"), allow_pickle=True
        )
    ...
    def predict(self, protein_sequence: str) -> Dict[str, numpy.float32]:
        ...
        # type: ignore
        raw_outputs = (
            torch.nn.functional.sigmoid(self.model(embedding))
            .squeeze()
            .detach()
            .cpu()
            .numpy()
        ...
        outputs: Dict[str, numpy.float32] = {
            label: confidence for label, confidence in zip(self.labels, raw_outputs)
        }
        logger.successfully_predicted_gene_ontology()
        return outputs


def run_gene_ontology_prediction(
    request: RunGeneOntologyPredictionRequest,
) -> RunGeneOntologyPredictionResponse:
    try:
        memory_manager.change_status(str(request.job_id), JobStatusEnum.running)
        model = GeneOntologyPrediction(
            model_name="gene_ontology", gpu=torch.cuda.is_available()
        )
        model.load_model()
        ...
        return RunGeneOntologyPredictionResponse(
            go_confidence=[
                GoConfidenceResponse(name=name, confidence=confidence)
